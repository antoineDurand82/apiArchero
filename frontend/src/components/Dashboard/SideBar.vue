  ...
  <div class="sidebar">
    <ul>
      <li class="sidebar-li" v-for="link in links" :key="link.route">
        <router-link class="sidebar-li-link" :to="{name: 'modelsCRUD', params: {entityName: link.route}}">{{ link.name }}</router-link>
      </li>
    </ul>
  </div>
</template>
    ...
    links() {
      return map(filter(Object.keys(this.$store.state.entities), key => key !== '$name'), model => {
        ...
        while(name.search(/[a-z][A-Z]/) !== -1) {
          ...
          route = route.slice(0, search + 1) + '-' + route[search].toLowerCase() + route.slice(search + 2)
        }
